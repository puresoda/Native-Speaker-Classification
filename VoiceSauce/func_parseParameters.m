...
for k=1:length(paramlist)
    ...
    elseif (strcmp(paramlist{k}, 'H4*-2K*'))
        new_param_vec(func_getparameterlist('H4*-2K*')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
        new_param_vec(func_getparameterlist(FMTalgorithm)) = new_param_vec(func_getparameterlist(FMTalgorithm)) + 2;    
        new_param_vec(func_getparameterlist('H1, H2, H4')) = new_param_vec(func_getparameterlist('H1, H2, H4')) + 2;
        new_param_vec(func_getparameterlist('2K')) = new_param_vec(func_getparameterlist('2K')) + 2;
                
    elseif (strcmp(paramlist{k}, '2K*-5K'))
        new_param_vec(func_getparameterlist('2K*-5K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
        new_param_vec(func_getparameterlist(FMTalgorithm)) = new_param_vec(func_getparameterlist(FMTalgorithm)) + 2;
        new_param_vec(func_getparameterlist('2K')) = new_param_vec(func_getparameterlist('2K')) + 2;
        new_param_vec(func_getparameterlist('5K')) = new_param_vec(func_getparameterlist('5K')) + 2;
    ...
    elseif (strcmp(paramlist{k}, '2K'))
        new_param_vec(func_getparameterlist('2K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
        
    elseif (strcmp(paramlist{k}, '5K'))
        new_param_vec(func_getparameterlist('5K')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
        
    elseif (strcmp(paramlist{k}, 'Formants (Snack)'))
        new_param_vec(func_getparameterlist('Formants (Snack)')) = 1;

    elseif (strcmp(paramlist{k}, 'Formants (Praat)'))
        new_param_vec(func_getparameterlist('Formants (Praat)')) = 1;        
        
    elseif (strcmp(paramlist{k}, 'Formants (Other)') && VSData.vars.FormantsOtherEnable == 1)
        new_param_vec(func_getparameterlist('Formants (Other)')) = 1;
    ...
    elseif (strcmp(paramlist{k}, 'epoch, excitation strength (epoch, SoE)') )
        new_param_vec(func_getparameterlist('epoch, excitation strength (epoch, SoE)')) = 1;
        new_param_vec(func_getparameterlist(F0algorithm)) = new_param_vec(func_getparameterlist(F0algorithm)) + 2;
    end
...
if (exist(matfile, 'file'))
    ...
    if (mod(new_param_vec(func_getparameterlist('Formants (Snack)')), 2) == 0 && isfield(matdata, 'sF1'))
        if (length(matdata.sF1) == data_len)
            new_param_vec(func_getparameterlist('Formants (Snack)')) = 0;
        end
    end

    if (mod(new_param_vec(func_getparameterlist('Formants (Praat)')), 2) == 0 && isfield(matdata, 'pF1') && isfield(matdata, 'pF5'))
        if (length(matdata.sF1) == data_len)
            new_param_vec(func_getparameterlist('Formants (Praat)')) = 0;
        end
    end    
    
    if (mod(new_param_vec(func_getparameterlist('Formants (Other)')), 2) == 0 && isfield(matdata, 'oF1'))
        if (length(matdata.sF1) == data_len)
            new_param_vec(func_getparameterlist('Formants (Other)')) = 0;
        end
    end    
    ...
    if (mod(new_param_vec(func_getparameterlist('2K')), 2) == 0 && isfield(matdata, 'H2K'))
        if (length(matdata.H2K) == data_len)
            new_param_vec(func_getparameterlist('2K')) = 0;
        end
    end

    if (mod(new_param_vec(func_getparameterlist('5K')), 2) == 0 && isfield(matdata, 'H5K'))
        if (length(matdata.H5K) == data_len)
            new_param_vec(func_getparameterlist('5K')) = 0;
        end
    end
end
...
if (new_param_vec(func_getparameterlist('Formants (Snack)')) == 1)
    new_paramlist{cnt} = 'Formants (Snack)';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('Formants (Praat)')) == 1)
    new_paramlist{cnt} = 'Formants (Praat)';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('Formants (Other)')) == 1 && VSData.vars.FormantsOtherEnable == 1)
    new_paramlist{cnt} = 'Formants (Other)';
    cnt = cnt + 1;
end
...
if (new_param_vec(func_getparameterlist('2K')) == 1)
    new_paramlist{cnt} = '2K';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('5K')) == 1)
    new_paramlist{cnt} = '5K';
    cnt = cnt + 1;
end
...
if (new_param_vec(func_getparameterlist('H4*-2K*')) == 1)
    new_paramlist{cnt} = 'H4*-2K*';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('2K*-5K')) == 1)
    new_paramlist{cnt} = '2K*-5K';
    cnt = cnt + 1;
end

if (new_param_vec(func_getparameterlist('epoch, excitation strength (epoch, SoE)')) == 1)
        new_paramlist{cnt} = 'epoch, excitation strength (epoch, SoE)';
    cnt = cnt + 1;
end
